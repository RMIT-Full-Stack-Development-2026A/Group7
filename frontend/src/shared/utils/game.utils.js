...
export function createsLineWithBlockedEnds(board, row, col, player, targetLength, blockedEndsTarget) {
  const testBoard = board.map((boardRow) => [...boardRow])
  testBoard[row][col] = player
  ...
  for (const { dr, dc } of directions) {
    ...
    while (step <= 4) {
      ...
      if (testBoard[newRow][newCol] !== null) blockedEnds += 1
      break
    }
    ...
    if (count >= targetLength && blockedEnds === blockedEndsTarget) {
      return true
    }
  }
...
// AI move selection moved to ai-moves.utils.js — re-exported for callers.
export { getAIMove } from './ai-moves.utils.js'


export function getWinningTiles(board, row, col, player) {
  const size = board.length
  const collectInDirection = (tiles, dr, dc, sign) => {
    for (let step = 1; step <= 4; step += 1) {
      const newRow = row + dr * step * sign
      const newCol = col + dc * step * sign
      if (newRow < 0 || newRow >= size || newCol < 0 || newCol >= size) return
      if (board[newRow][newCol] !== player) return
      tiles.push({ row: newRow, col: newCol })
    }
  }
  for (const { dr, dc } of directions) {
    const tiles = [{ row, col }]
    collectInDirection(tiles, dr, dc, 1)
    collectInDirection(tiles, dr, dc, -1)
