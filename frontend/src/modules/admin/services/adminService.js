...
// Online rooms = rooms that exist and haven't been completed.
// `available`, `full`, and `in-battle` all count as live, visible rooms in the lobby/system.
export const fetchOnlineRooms = async ({ q = '' } = {}) => {
  const statuses = ['available', 'full', 'in-battle']
  const results = await Promise.all(statuses.map((status) => fetchGames({ status, q })))

  const errored = results.find((response) => !response.ok || !response.data?.ok)
  if (errored) {
    return errored
  }

  const merged = results.flatMap((response) => normalizeGamesPayload(response.data))
  const dedupedById = Array.from(new Map(merged.map((room) => [String(room._id), room])).values())
  dedupedById.sort((a, b) => {
    const aTime = new Date(b?.startedAt || b?.updatedAt || b?.createdAt || 0).getTime()
    const bTime = new Date(a?.startedAt || a?.updatedAt || a?.createdAt || 0).getTime()
    return aTime - bTime
  })
