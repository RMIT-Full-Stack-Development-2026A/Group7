...
export const buildPlayersFromRoom = (roomData, isAIGame, aiDifficulty, turnSelection) => {
  ...
  const formatAIName = (baseName, difficulty) => {
    if (!difficulty) return baseName
    return `${baseName} (${difficulty})`
  }

  if (turnAssignments?.X && turnAssignments?.O) {
    const xIsAI = turnAssignments.X.type === 'ai'
    const oIsAI = turnAssignments.O.type === 'ai'
    const xDifficulty = turnAssignments.X.aiDifficulty || (xIsAI ? aiDifficulty : null)
    const oDifficulty = turnAssignments.O.aiDifficulty || (oIsAI ? aiDifficulty : null)
    const xBaseName = turnAssignments.X.name || DEFAULT_PLAYERS.X.name
    const oBaseName = turnAssignments.O.name || (isAIGame ? 'AI' : DEFAULT_PLAYERS.O.name)
    return {
      X: {
        userId: turnAssignments.X.userId || null,
        name: xBaseName,
        displayName: xIsAI ? formatAIName(xBaseName, xDifficulty) : xBaseName,
        avatar: resolveAvatarUrl(turnAssignments.X.avatar, { isAI: xIsAI, fallbackToDefault: false }),
        marker: turnAssignments.X.marker || 'X',
        markerColor: turnAssignments.X.markerColor || '',
      },
      O: {
        userId: turnAssignments.O.userId || null,
        name: oBaseName,
        displayName: oIsAI ? formatAIName(oBaseName, oDifficulty) : oBaseName,
        avatar: resolveAvatarUrl(turnAssignments.O.avatar, { isAI: oIsAI, fallbackToDefault: false }),
        marker: turnAssignments.O.marker || 'O',
        markerColor: turnAssignments.O.markerColor || '',
      },
    }
  }
  ...
  const firstIsAI = firstPlayer?.type === 'ai'
  const secondIsAI = secondPlayer?.type === 'ai' || (isAIGame && !secondPlayer)
  const firstDifficulty = firstPlayer?.aiDifficulty || (firstIsAI ? aiDifficulty : null)
  const secondDifficulty = secondPlayer?.aiDifficulty || (secondIsAI ? aiDifficulty : null)
  const firstBaseName = firstPlayer?.name || DEFAULT_PLAYERS.X.name
  const secondBaseName = secondPlayer?.name || (isAIGame ? 'AI' : DEFAULT_PLAYERS.O.name)

  return {
    X: {
      userId: firstPlayer?.userId || firstPlayer?._id || null,
      name: firstBaseName,
      displayName: firstIsAI ? formatAIName(firstBaseName, firstDifficulty) : firstBaseName,
      avatar: resolveAvatarUrl(firstPlayer?.avatar || firstPlayer?.avatarUrl, { isAI: firstIsAI, fallbackToDefault: false }),
      marker: markerTermToSymbol(firstPlayer?.marker) || markerLabels.X,
      markerColor: firstPlayer?.markerColor || '',
    },
    O: {
      userId: secondPlayer?.userId || secondPlayer?._id || null,
      name: secondBaseName,
      displayName: secondIsAI ? formatAIName(secondBaseName, secondDifficulty) : secondBaseName,
      avatar: resolveAvatarUrl(secondPlayer?.avatar || secondPlayer?.avatarUrl, { isAI: secondIsAI, fallbackToDefault: false }),
      marker: markerTermToSymbol(secondPlayer?.marker) || markerLabels.O,
      markerColor: secondPlayer?.markerColor || '',
    },
  }
}
...
export const buildRemoteParticipantPayloads = ({ localTurnPlayers, players, remotePlayers = null }) => {
  ...
  return ['X', 'O'].map((symbol, index) => {
    const remoteSource = remotePlayers?.[symbol] || {}
    return {
      ...
      order: index + 1,
      isAI: Boolean(remoteSource.isAI),
      aiDifficulty: remoteSource.isAI ? (remoteSource.aiDifficulty || null) : null,
    }
  })
}
