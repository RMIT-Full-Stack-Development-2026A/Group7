...
export const resolveAuthIdentity = async () => {
  ...
  const fetchProfileIdentity = async (identifier, useToken = true) => {
    const profileUrl = identifier
      ? `${API_BASE_URL}/profile?userId=${encodeURIComponent(identifier)}`
  ...
  const mergeAndPersist = (profileIdentity) => {
  ...
  try {
    const profileIdentity = await fetchProfileIdentity(storedIdentity.userId, true);
    const profileLooksLikeRequestedUser = profileIdentity
      && (!storedIdentity.username || profileIdentity.username === storedIdentity.username);

    if (profileLooksLikeRequestedUser) {
      return mergeAndPersist(profileIdentity);
    }

    const stableIdentifier = storedIdentity.username || storedIdentity.email;
    if (stableIdentifier) {
      const refreshedIdentity = await fetchProfileIdentity(stableIdentifier, false);
      const refreshedLooksLikeRequestedUser = refreshedIdentity
        && (!storedIdentity.username || refreshedIdentity.username === storedIdentity.username);

      if (refreshedLooksLikeRequestedUser) {
        return mergeAndPersist(refreshedIdentity);
      }
    }
  } catch {
