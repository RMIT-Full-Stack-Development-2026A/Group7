...
const wasRoomDeleted = (room) => room?.$locals?.gameroomDeleted === true
const getAuthenticatedUserId = (req) => req.user?.userId || req.user?.id || null

const resolveRequestUserId = (req) => gameroomService.resolveUserIdFromRequest({
...
const addPlayerToGameroom = async (req, res, next) => {
  try {
    const authenticatedUserId = await resolveRequestUserId(req)
...
const removePlayerFromGameroom = async (req, res, next) => {
  try {
    ...
    const roomResponse = toGameroomResponse(room)
    if (wasRoomDeleted(room)) {
      emitGameroomEvent(room.roomId, 'room-deleted', roomResponse)
    } else if (shouldEmitPlayersChanged(room)) {
      emitGameroomEvent(room.roomId, 'room-updated', roomResponse)
    }
...
const startGameroom = async (req, res, next) => {
  try {
    ...
    if (activePlayers.some((player) => !player.marker)) {
      return next(new ErrorResponse('Every player must choose a marker before starting the game.', 400))
    }
...
const getGameroomChatMessages = async (req, res, next) => {
  try {
    const messages = await gameroomService.getGameroomChatMessages(req.params.id)
...
const addGameroomChatMessage = async (req, res, next) => {
  try {
    const message = await gameroomService.addGameroomChatMessage(req.params.id, req.body)
    const room = await gameroomService.getGameroomById(req.params.id)

    emitGameroomEvent(room.roomId, 'chat-message', {
      roomId: room.roomId,
      message,
    })
